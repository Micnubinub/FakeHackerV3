...
public class Commands {
    ...
    public void checkCommand(String userCmd) {
        ...
        if (!userCmd.equals("repeat")) {
            lastCom = userCmd;
        } else if (FileManagerFragment.handleConsoleCommand(userCmd)) {
            return;
        } else if (RemoteFragment.handleConsoleCommand(userCmd)) {
            return;
        }

        if (userCmd.startsWith("chat")) {
